...
def compute_operations(data, test_file_data):
    tri_count, bi_count = count_n_grams(data=data)
    values, alpha_values = compute_k_fold_validation(data=data)
    avg_perplexity = np.sum(values) / len(values)
    best_alpha = np.sum(alpha_values) / len(alpha_values)
    perplexity_test_file = calculate_perplexity(data=test_file_data, tri_count=tri_count, bi_count=bi_count, alpha=1)
    perplexity_test_file_alpha = calculate_perplexity(data=test_file_data, tri_count=tri_count, bi_count=bi_count, alpha=best_alpha)

    return values, best_alpha, perplexity_test_file, perplexity_test_file_alpha

# Plot grid for perplexity values
def plot_grid(en_values, es_values, de_values):
    x_grid = np.arange(start=0, stop=10, step=1)
    plt.clf()
    plt.plot(x_grid, en_values, 'b-', label='English Model')
    plt.plot(x_grid, es_values, 'r-', label='Spanish Model')
...
# Plot bar for perplexity values
def plot_bar(en_perplexity, es_perplexity, de_perplexity):
    x_grid_prime = np.arange(start=0, stop=3, step=1)
    yy = [en_perplexity, es_perplexity, de_perplexity]
    plt.clf()
    plt.bar(1, en_perplexity, color="blue", label='English Model: ' + str('%.3f' % en_perplexity))
    plt.bar(2, es_perplexity, color="red", label='Spanish Model: ' + str('%.3f' % es_perplexity))
    plt.bar(3, de_perplexity, color="green", label='German Model: ' + str('%.3f' % de_perplexity))
    plt.legend()
    plt.show()
